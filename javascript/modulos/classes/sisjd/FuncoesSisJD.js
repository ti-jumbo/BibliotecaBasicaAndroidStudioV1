...
const {default:fnreq} = await import('/sjd/javascript/modulos/classes/requisicao/FuncoesRequisicao.js?'+window.version_loads).catch((error)=>{console.log(error);alert("Erro de carregamento de modulos.\nPor favor, tente atualizar a pagina novamente com Ctrl+F5.\nSe o erro persistir mesmo assim, tente limpar o historico do navegador.\nAinda Persistindo, contacte o administrador da pagina.");});
...
const {default:fnmat} = await import('/sjd/javascript/modulos/classes/matematica/FuncoesMatematica.js?'+window.version_loads).catch((error)=>{console.log(error);alert("Erro de carregamento de modulos.\nPor favor, tente atualizar a pagina novamente com Ctrl+F5.\nSe o erro persistir mesmo assim, tente limpar o historico do navegador.\nAinda Persistindo, contacte o administrador da pagina.");});
const {default:fngraf} = await import('/sjd/javascript/modulos/classes/grafico/FuncoesGrafico.js?'+window.version_loads).catch((error)=>{console.log(error);alert("Erro de carregamento de modulos.\nPor favor, tente atualizar a pagina novamente com Ctrl+F5.\nSe o erro persistir mesmo assim, tente limpar o historico do navegador.\nAinda Persistindo, contacte o administrador da pagina.");});
const {default:fnobj} = await import('/sjd/javascript/modulos/classes/objeto/FuncoesObjeto.js?'+window.version_loads).catch((error)=>{console.log(error);alert("Erro de carregamento de modulos.\nPor favor, tente atualizar a pagina novamente com Ctrl+F5.\nSe o erro persistir mesmo assim, tente limpar o historico do navegador.\nAinda Persistindo, contacte o administrador da pagina.");});
const {default:fnarr} = await import('/sjd/javascript/modulos/classes/array/FuncoesArray.js?'+window.version_loads).catch((error)=>{console.log(error);alert("Erro de carregamento de modulos.\nPor favor, tente atualizar a pagina novamente com Ctrl+F5.\nSe o erro persistir mesmo assim, tente limpar o historico do navegador.\nAinda Persistindo, contacte o administrador da pagina.");});
...
class FuncoesSisJD{    
    ...
    static getInstance(){
        if (FuncoesSisJD.#instance == null) {
            FuncoesSisJD.#instance = new FuncoesSisJD();
            window.fnsisjd = FuncoesSisJD.#instance;
        }
        return FuncoesSisJD.#instance;
    }
    ...
    verificar_login(params) {
        try {
            ...
            if (params.comhttp.retorno.dados_retornados.conteudo_html.trim().toLowerCase() === 'logado') {
                ...
                localStorage.setItem("usuariosis",JSON.stringify(params.comhttp.retorno.dados_retornados.dados.usuariosis));
                let params_req = {
                    async: false,
    ...
    inserir_visao_pesquisa(params){
        try{
            ...
            let veio_selecionado = false;
            if (typeof params.selecionado !== "undefined" && params.selecionado.trim().length > 0) {
                veio_selecionado = true;
            } else {
            ...
            } else if (params.elemento.hasClass("btn_img_add_geral") || params.elemento.hasClass("row")) {
                if (!veio_selecionado) {
        ...
        try{			
            ...
            if (typeof params !== "undefined" && params !== null) {
                ...
                if (typeof elem !== "undefined" && elem !== null && elem.length) {                                       
                    let div = elem.closest("div.accordion-body").children("div:first");
                    if (typeof div === "undefined" || div === null || !div.length) {
                        div	= elem.closest('div.card').closest("div.row");
                    }
                    if (typeof div === "undefined" || div === null || !div.length) {
                        div = elem;
                    }                
                    ...
                    let div_periodos = div.closest("div#periodos");
                    ...
                    params.permite_incluir = fnjs.como_booleano(fnjs.first_valid([params.permite_incluir,div_periodos.attr("permite_incluir")]));
    ...
    criar_controle_combobox_visao(params) {
        try {            
            ...
            params.permite_incluir = fnjs.como_booleano(fnjs.first_valid([params.permite_incluir,true]));            
            params.permite_excluir = fnjs.como_booleano(fnjs.first_valid([params.permite_excluir,true]));
    ...
    carregar_valores_inicio(){
        try {
            ...
            FuncoesSisJD.getInstance().requisitar_valores_inicio();
    ...
    */
    ...
    destacar_celulas_zeradas(params) {
        try {
            ...
            if (!linhas.length) {
                params.contador_recursao++;
                if (params.contador_recursao <= 100) {
                    setTimeout(window.fnsisjd.destacar_celulas_zeradas,2000,params);
                    return;
                }
            }
    ...
    carregar_dados_campanha_consultar(obj){
        try{
            ...
            if ((linha.attr("status") || "normal") === "normal" && !cel.hasClass("cel_cmd")) {			
                ...
                fnjs.obterJquery("div.div_dados_campanhas_consultar_container_corpo1").html(''
                    +'<button class="botao_padrao" onclick="window.fnsisjd.imprimir_conteudo_elemento(this.parentNode)">'
                        +'Imprimir Relatorio'
                    +'</button>'
                    +'<br />'
                    +'<div class="div_consultar_objet_gerais_camp">'
                    +'</div>'
                    +'<br /><br /><div class="div_consultar_objet_gerais_subcampanhas"><h3>Sub Campanhas:</h3><br /></div>'
                );
    ...
    */	
    ...
    preencher_dados_padrao_linha_objetivos_campanha(params) {
		try {
			...
			if (typeof vars.sjd.visoes === 'undefined' || vars.sjd.visoes.length === 0) {
				requisitar_visoes({
			...
			if (celslin.eq(indcampo).children("div.div_combobox").length) {
				...
				opcoes_combobox = {};			
				opcoes_combobox.opcoes_texto_opcao = vars.sjd.visoes;
				...
				valor = (celslin.eq(indcampo).children("input[type=text]").eq(0).val()||celslin.eq(indcampo).text()||"").trim().toLowerCase();
				if (valor.length > 0) {
					opcoes_combobox.selecionados.push(vars.sjd.visoes.join(",").trim().toLowerCase().split(",").indexOf(valor));
				}
    ...
    salvar_condicionantes(obj){
        try{
           ...
           divs_operacoes = divs_condicionantes.filter('div.operacao');
            ...
            condicionantes = condicionantes.join(vars.sepn1).replace(/,/g,vars.subst_virg);
           ...
           fnreq.atualizar_dados_sql({opcoes:opcoes});
        }catch(e){
            console.log(e);
            alert(e.message || e);
        }			
		...
		try {
			...
			if (vars.navegador === "iexplorer") {
				...
				popupImprimir.onload = function(){			
                    ...
                    $(popupImprimir.document.body).append('<script type="text/javascript" src="/jquery/3.6.0/jquery-3.6.0.min.js?12.33" charset="UTF-8"></script>');
	...
	pesquisar_filtro_padrao(params){
		try {
			...
			if (condicionantes.length) {
				comhttp.requisicao.requisitar.qual.condicionantes.push("condicionantes=" + condicionantes.join(vars.sepn1));
			}
    ...
    inserirGraficoVolumeInicio(params){
        try {
            ...
            function desenharGraficosVolumeInicio() {
                ...
                if (!localStorage.getItem(vars.nameLocalStorageDataGraphicInit + fndt.getAno() + "_" + (localStorage.getItem("codusur")||""))) {
                    for(let i = 0; i < qt; i++) {
                        localStorage.setItem(vars.nameLocalStorageDataGraphicInit+fndt.getAno()+"_"+(localStorage.getItem("codusur")||""),params.comhttp.retorno.dados_retornados.conteudo_html.dados.join(";"));
                    ...
                    let valores_armazenados = localStorage.getItem(vars.nameLocalStorageDataGraphicInit + fndt.getAno() + "_" + (localStorage.getItem("codusur")||""));
                    valores_armazenados = valores_armazenados.split(";");                    
                    let valorMesAtual = 0;
                    let valorMesAnterior = null;
                    let data = new Date();
                    if ([1,2,3,4,5,6,7].indexOf(data.getDate()) > -1) {
                        valorMesAnterior = parseFloat(params.comhttp.retorno.dados_retornados.conteudo_html.dados[0] || 0);
                        valorMesAtual = parseFloat(params.comhttp.retorno.dados_retornados.conteudo_html.dados[1] || 0);
                    } else {
                        valorMesAtual = parseFloat(params.comhttp.retorno.dados_retornados.conteudo_html.dados[0] || 0);
                    }
                    for(let i = 0; i < qt; i++) {
                        ...
                        if (i == (fndt.getMes()-2) && valorMesAnterior != null) {
                            valor2 = valorMesAnterior;
                    ...
                    localStorage.setItem(vars.nameLocalStorageDataGraphicInit+fndt.getAno()+"_"+(localStorage.getItem("codusur")||""),valores_armazenados.join(";"));
                }
    ...
    carregarGraficoVolumeInicio(){
        try {
            ...
            if (!localStorage.getItem(vars.nameLocalStorageDataGraphicInit + fndt.getAno() + "_" + (localStorage.getItem("codusur")||""))) {			
                ...
                let data = new Date();
                if ([1,2,3,4,5,6,7].indexOf(data.getDate()) > -1) {
                    comhttp.requisicao.requisitar.qual.objeto = "dados_grafico_volume_inicio_mes_atual_e_anterior";
                } else {
        ...
        try {
            ...
            google.charts.setOnLoadCallback(function(){
                ...
                if (params.inicio == true) {
                    ...
                    for(let i = 0; i < qt; i++) {
                        linha = [];
                        linha.push(vars.constantes.meses_abrev[i]);
...
export default FuncoesSisJD.getInstance(); 