<?php
	...
	class FuncoesAtualizacao extends ClasseBase{
		public static function criar_arquivo_recurso_todos(&$comhttp,&$opcoes) {
			try {
				if (count($opcoes) > 0) {
					//print_r($opcoes["atualizacao"]); //exit();
						...
						if (property_exists($tabela,"sub")) {
							foreach($tabela->sub as $chave_campo => $campo) {
								...
								foreach ($campo as $chave_campo_campo => $valor_campo) {
									...
									$tipo_campo = strtolower(trim((isset($valores_campos[$chave_campo]["tipodado"])?$valores_campos[$chave_campo]["tipodado"]:"varchar2")));
									foreach(VariaveisSql::getInstancia()::$dados_conexoes->{$opcoes["atualizacao"]["nome_conexao"][0]}->driver->types as $chave_tipo_dado_driver => $tipo_dado_driver) {
										if (strcasecmp(trim($chave_tipo_dado_driver),$tipo_campo) == 0 || 
											strcasecmp(trim($tipo_dado_driver->name),$tipo_campo) == 0 || 
											in_array(strtolower(trim($tipo_campo)),$tipo_dado_driver->synonyms)
										) {
											$tipo_campo = $tipo_dado_driver->name;
											break;
										} 
									}									
									...
									$texto_campos[] = $nome_campodb." ".$tipo_campo." ".$valorpadrao." ".$chaveprimaria . " " . $unico." ".$permitenulo . " " . $chaveestrangeira;
							}
		...
		public static function dados_campo_como_sql($campo) {
			...
			if (isset($campo["column_name"])) {
				...
				*/
				$dados_campo_como_sql["nomecampodb"] = strtolower(trim((isset($campo["nomecampodb"])?$campo["nomecampodb"]:"novocampo")));
				$dados_campo_como_sql["tipodado"] = strtolower(trim((isset($campo["tipodado"])?$campo["tipodado"]:"varchar2")));
				$dados_campo_como_sql["tamanho"] = strtolower(trim((isset($campo["tamanho"])?$campo["tamanho"]:"")));
				$dados_campo_como_sql["precisao"] = strtolower(trim((isset($campo["precisao"])?$campo["precisao"]:"")));
				$dados_campo_como_sql["chaveprimaria"] = strtolower(trim((isset($campo["chaveprimaria"])?$campo["chaveprimaria"]:"0")));
				$dados_campo_como_sql["unico"] = strtolower(trim((isset($campo["unico"])?$campo["unico"]:"0")));
				$dados_campo_como_sql["permitenulo"] = strtolower(trim((isset($campo["permitenulo"])?$campo["permitenulo"]:"1")));
				$dados_campo_como_sql["valorpadrao"] = strtolower(trim((isset($campo["valorpadrao"])?$campo["valorpadrao"]:"")));	
				$dados_campo_como_sql["chaveestrangeira"] = strtolower(trim((isset($campo["chaveestrangeira"])?$campo["chaveestrangeira"]:"")));		
			}
		...
		public static function montar_expressao_campo_sql($dados_campo,$opcoes) {
			...
			$dados_campo["tipodado"] = FuncoesSql::getInstancia()->tipo_como_tipo_driver($types_driver,$dados_campo["tipodado"]);
			/*foreach($types_driver as $chave_tipo_dado_driver => $tipo_dado_driver) {
				if (strcasecmp(trim($chave_tipo_dado_driver),$dados_campo["tipodado"]) == 0 || 
					strcasecmp(trim($tipo_dado_driver->name),$dados_campo["tipodado"]) == 0 || 
					in_array(strtolower(trim($dados_campo["tipodado"])),$tipo_dado_driver->synonyms)
				) {
												...
												foreach ($campos_existentes as &$campo_existente) {
													...
													foreach ($tabela["campos"] as &$campo_tab) {
														...
														if (strcasecmp(trim($campo_existente["column_name"]),trim($campo_tab["nomecampodb"])) == 0) {
															...
															//print_r($dados_campo_existente); exit();
		...
		public static function preparar_registro_atualizacao($atualizacao) {
			if ($atualizacao !== null) {
		...
		public static function procurar_comando_sql_arquivo(&$comhttp,&$opcoes) {	
			...
			$atualizacao = self::preparar_registro_atualizacao($atualizacao);
			...
			try {
				...
				if (FuncoesSql::getInstancia()->tabela_existe($tabela,true)) {		
					...
					if (count($dados) > 0) {
						...
						foreach($dados as $lin) {			
							//print_r($lin); exit();
							$opcoes["atualizacao"] = self::preparar_registro_atualizacao($lin);
							//print_r($opcoes["atualizacao"]); exit();
