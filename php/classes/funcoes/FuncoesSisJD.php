<?php
	...
	class FuncoesSisJD extends ClasseBase{		
		...
		/**
		 * login into system
		 * @param string|int $code - the code of the user
		 * @param string|int $password - the password of the user
		 * @return bool - if logged or not
		 */
		public static function login(string|int $code, string|int $password) : bool{
			$result = false;
			$cmd = "select count(1) from sjdusuariosis where codusuariosis = '$code' and lower(senha) = lower('$password')";			
			$result = FuncoesSql::getInstancia()->executar_sql($cmd,'fetch',\PDO::FETCH_COLUMN);
			$result = FuncoesConversao::como_boleano($result);
			return $result;
		}
			...
			if(isset($arr_dado)){
				...
				switch($tipo_arr_dado){
					case 'string':
						...
						FuncoesBasicasRetorno::mostrar_msg_sair('Erro na requisicao: tipo de arr dado global não suportado: '.$tipo_arr_dado,__FILE__,__FUNCTION__,__LINE__);
						break;
					case 'array':
						$alvo=null;
						$cont=0;
					...
					default:
						...
						FuncoesBasicasRetorno::mostrar_msg_sair('Erro na requisicao de dados sql: tipo de arr dado global não definido: '.json_encode($comhttp->requisicao->condicionantes),__FILE__,__FUNCTION__,__LINE__);
						break;
				}
			}
			return $retorno;
			...
			if(isset($arr_dado)){
				...
				switch($tipo_arr_dado){
					case 'string':
						...
						FuncoesBasicasRetorno::mostrar_msg_sair('Erro na requisicao: tipo de arr dado global não definido: '.$tipo_arr_dado,__FILE__,__FUNCTION__,__LINE__);
						break;
					case 'array':
						$alvo=null;
						$cont=0;
		...
		public static function dados_literais(&$comhttp){
			...
			switch(strtolower(trim($tipo_objeto))){
				...
				default:
					...
					FuncoesBasicasRetorno::mostrar_msg_sair('Erro na requisicao: tipo dados literais não definido: '.$comhttp->requisicao->requisitar->qual->tipo_objeto,__FILE__,__FUNCTION__,__LINE__);
					break;			
			}
			return $retorno;
		}
						...
						switch (strtolower(trim($comhttp->requisicao->requisitar->qual->condicionantes['relatorio']))) {
							...
							case 'positivacaopersonalizada':
								...
								foreach( $arr_dados as $ilin => $lin) {
									...
									foreach( $arr_dados[ $ilin ] as $k => $v) {
										if ($k > $ka) {//ka = 2 k = 3
											for($ka = $ka ; $ka < $k ; $ka ++) {
												$arr_dados[ $ilin ][ $ka ] = $valor_branco;
											}
										}
										$ka = $k + 1;	
									}
							...
							case 'entregas':										
								...
								select
									...
									c.observacao,
									(select count(1) from sjdacompentreganotas n where n.numcar = c.numcar and nvl(n.statusentrega,0) = 0) as \"NFs A Entregar\",
									(select count(1) from sjdacompentreganotas n where n.numcar = c.numcar and n.statusentrega = 2) as \"NFs Entregue\",
									(select count(1) from sjdacompentreganotas n where n.numcar = c.numcar and n.statusentrega = 3) as \"Dev. Parcial\",
									(select count(1) from sjdacompentreganotas n where n.numcar = c.numcar and n.statusentrega = 4) as \"Dev. Total\",
									(
										select 
											sum(nvl(i.qtentregue,0) * nvl(i.valorunitario,0)) 
										from 
											sjdacompentreganotas n 
											join sjdacompentregaprod i on i.chavenfe = n.chavenfe 
										where 
											n.numcar = c.numcar 
											and n.statusentrega = 2
									) as \"Vl. Entregue\",
									(
										select 
											sum(nvl(i.qtentregue,0) * nvl(i.valorunitario,0)) 
										from 
											...
											join jumbo.pcnfsaid s on (
												s.numcar = n.numcar 
												and s.chavenfe = n.chavenfe
												and s.codcob in ('D','CHQ')
											)
											join sjdacompentregaprod i on i.chavenfe = n.chavenfe 
										where 
											n.numcar = c.numcar 
											and n.statusentrega in (2,3)
									) as \"Vl. Devido\",
									(select sum(nvl(pgd.valor,0)) from sjdacompentregapagcar pgd where pgd.numcar = c.numcar and lower(trim(pgd.formapagamento)) = 'deposito') as \"Vl. Depositado\",
									(
										select 
											sum(nvl(pgp.valor,0))
										from 
											sjdacompentregapag pgp 
											join sjdacompentreganotas n on (
												n.chavenfe = pgp.chavenfe
											)
										where 
											n.numcar = c.numcar
											and lower(trim(pgp.formapagamento)) = 'pix'
									) as \"Vl. Pix\",
									(select sum(nvl(pgd.valor,0)) from sjdacompentregapagcar pgd where pgd.numcar = c.numcar and lower(trim(pgd.formapagamento)) = 'malote') as \"Vl. Malote\",
									(
										select 
											sum(nvl(i.qtentregue,0) * nvl(i.valorunitario,0)) 
										from 
										...
										(select sum(nvl(pgd.valor,0)) from sjdacompentregapagcar pgd where pgd.numcar = c.numcar and lower(trim(pgd.formapagamento)) = 'deposito')
										+(select sum(nvl(pgd.valor,0)) from sjdacompentregapagcar pgd where pgd.numcar = c.numcar and lower(trim(pgd.formapagamento)) = 'malote')
										+(
											select 
												sum(nvl(pgp.valor,0))
											from 
								...
								if (gettype($comhttp->requisicao->requisitar->qual->condicionantes['condicionantes']) === 'string' && $comhttp->requisicao->requisitar->qual->condicionantes['condicionantes'] !== null) {
									...
									if (count($cond_rca) > 0 ) {
										$condicionantes[] = 'exists(select 1 from jumbo.pcnfsaid s where s.numcar = c.numcar and ('.implode(' or ', $cond_rca).'))';
									}
								}
							...
							case 'dados_grafico_volume_inicio_mes_atual':
								...
								//print_r($comhttp->retorno->dados_retornados['dados']);exit();
								break;
							case 'dados_grafico_volume_inicio_mes_atual_e_anterior':
								$comhttp->requisicao->requisitar->qual->objeto = 'empresa';
								$dtini = FuncoesData::data_primeiro_dia_mes_atual(FuncoesData::dataBr());
								$dtfim = FuncoesData::UltDiaMes(FuncoesData::dataBr());
								$dtiniant = FuncoesData::data_primeiro_dia_mes_anterior(FuncoesData::dataBr());
								$dtfimant = FuncoesData::UltDiaMes($dtiniant);
								$datas = [$dtiniant,$dtfimant,$dtini,$dtfim];
								//print_r($datas);exit();
		...
		public static function incluir_entrega_simples(&$comhttpsimples) {				
			try {
				...
				foreach($cursorNota['fields'] as $campo) {
					if (property_exists($entrega_recebida,$campo)){
						$camposInsert[$campo] = $entrega_recebida->{$campo};	
						if (in_array($campo,['valor'])) {
							$camposInsert[$campo] = FuncoesConversao::como_numero($camposInsert[$campo]);
						}					
					}
				}	
				self::prepararValoresInclusao($camposInsert);
				...
				foreach($entrega_recebida->itens as $item) {
					...
					foreach($cursorItens['fields'] as $campo) {
						if (property_exists($item,$campo)) {
							...
							if (in_array($campo,['valor'])) {
								$camposInsertItem[$campo] = FuncoesConversao::como_numero($camposInsertItem[$campo]);
							}
						}
				...
				foreach($entrega_recebida->pagamentos as $pag) {
					...
					foreach($cursorPags['fields'] as $campo) {
						if (property_exists($pag,$campo)) {
							$camposInsertPags[$campo] = $pag->{$campo};
							if (in_array($campo,['valor'])) {
								$camposInsertPags[$campo] = FuncoesConversao::como_numero($camposInsertPags[$campo]);
							}
						}
					}
					//print_r($camposInsertPags);exit();
